...
#
#   siehe auch
#   http://www.knight-of-pi.org/home-automation-with-rpi-rf_send-and-rpi-rf_receive-for-433mhz-rc-switches/
#
#   Modifikation PBX: eingefügt:   
#       GPIO.setwarnings(False)  
#       Instanz von MyPrint Klasse wird übergaben bei _init_
#---------------------------------------------------------------------
...
class RFDevice:
    ...
    def __init__(self, gpio, mprint,
        ...
        GPIO.setwarnings(False)
        self.myprint (DEBUG_LEVEL1, "rpt-rf: Using GPIO " + str(gpio))
    ...
    def cleanup(self):
        ...
        if self.rx_enabled:
            self.disable_rx()
        self.myprint (DEBUG_LEVEL2, "rpt-rf: Cleanup")
        GPIO.cleanup()

    def enable_tx(self):
        """Enable TX, set up GPIO."""
        if self.rx_enabled:
            self.myprint (DEBUG_LEVEL3, "rpt-rf: RX is enabled, not enabling TX")
            return False
        if not self.tx_enabled:
            self.tx_enabled = True
            GPIO.setup(self.gpio, GPIO.OUT)
            self.myprint (DEBUG_LEVEL3, "rpt-rf: TX enabled")
        return True
    ...
    def disable_tx(self):
        ...
        if self.tx_enabled:
            ...
            self.tx_enabled = False
            self.myprint (DEBUG_LEVEL3, "rpt-rf: TX disabled")
        return True
    ...
    def tx_code(self, code, tx_proto=None, tx_pulselength=None, tx_length=None):
        ...
        self.myprint (DEBUG_LEVEL3, "rpt-rf: TX code: " + str(code))
        return self.tx_bin(rawcode)

    def tx_bin(self, rawcode):
        """Send a binary code."""
        self.myprint (DEBUG_LEVEL3, "rpt-rf: TX bin: " + str(rawcode))
    ...
    def tx_l0(self):
        ...
        if not 0 < self.tx_proto < len(PROTOCOLS):
            self.myprint (DEBUG_LEVEL3, "rpt-rf: Unknown TX protocol")
            return False
    ...
    def tx_waveform(self, highpulses, lowpulses):
        """Send basic waveform."""
        if not self.tx_enabled:
            self.myprint (DEBUG_LEVEL3, "rpt-rf: TX is not enabled, not sending data")
            return False
    ...
    def enable_rx(self):
        ...
        if self.tx_enabled:
            self.myprint (DEBUG_LEVEL3, "rpt-rf: TX is enabled, not enabling RX")
            return False
        if not self.rx_enabled:
            ...
            self.myprint (DEBUG_LEVEL3, "rpt-rf: RX enabled")
        return True
    ...
    def disable_rx(self):
        ...
        if self.rx_enabled:
            ...
            self.rx_enabled = False
            self.myprint (DEBUG_LEVEL3, "rpt-rf: RX disabled")
        return True
                ...
                if self._rx_repeat_count == 2:
                    for pnum in range(1, len(PROTOCOLS)):
                        if self._rx_waveform(pnum, self._rx_change_count, timestamp):
                            self.myprint (DEBUG_LEVEL3, "rpt-rf: RX code " + str(self.rx_code))
                            break
                    self._rx_repeat_count = 0
            self._rx_change_count = 0
